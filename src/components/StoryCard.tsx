...
export const StoryCard = forwardRef<HTMLDivElement, StoryCardProps>(function StoryCard(
  { dinnerName, imageUrl, vibeScore, vibeCategory, className = '' },
  ref
) {
  return (
    <div
    ...
    >
      ...
      <div className="absolute inset-0">
        {imageUrl ? (
          imageUrl.startsWith('data:') ? (
            ...
            <img src={imageUrl} alt={dinnerName} className="w-full h-full object-cover" />
          ) : (
            <Image src={imageUrl} alt={dinnerName} fill className="object-cover" />
          )
        ) : (
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 p-6 bg-gradient-to-t from-[#FAF9F7] via-[#FAF9F7]/95 to-transparent pt-24">
        ...
        {vibeScore && (
          <div className="flex items-center gap-3 mb-2">
            <span className="text-3xl font-bold text-[#E8734A]">{vibeScore}</span>
            {vibeCategory && (
      ...
      <div className="absolute top-4 right-4 z-20">
        <div className="bg-white/80 backdrop-blur-sm rounded-full px-3 py-1.5">
          <p className="text-[#E8734A] text-xs font-semibold">CharcuterME</p>
        </div>
      </div>
    </div>
  );
